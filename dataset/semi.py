...
class SemiDataset(Dataset):
    def __init__(self, name, root, mode, size, labeled_id_path=None, unlabeled_id_path=None, pseudo_mask_path=None, fixed_size=(512, 512)):
        ...
        :param mode: train: supervised learning only with labeled images, no unlabeled images are leveraged.
                     unlabeled: load images only, dummy ignore mask (for teacher pseudo-labeling).
    ...
    def __getitem__(self, item):
        id = self.ids[item]
        # .convert('RGB') is a no-op for the RGB Pascal/Cityscapes images but is
        # required for ADE20K, which contains grayscale (single-channel) images;
        # without it the tensor is [1,H,W] and broadcasting against the 3-channel
        # ImageNet normalisation fails ([1,518,518] vs [3,518,518]).
        ...
        if self.mode == 'unlabeled':
            # Dummy ignore mask — teacher will generate pseudo-labels
            mask = Image.fromarray(np.full((img.size[1], img.size[0]), 255, dtype=np.uint8))
        elif self.mode == 'train' or (self.mode == 'semi_train' and id in self.labeled_ids):
            mask = Image.open(os.path.join(self.root, id.split(' ')[1]))
        else:
