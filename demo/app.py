"""CW-BASS v2 interactive demo (Hugging Face Space) — Pascal VOC / Cityscapes / ADE20K.

Layout expected in the Space repo:
    app.py, labels.py, requirements.txt
    model/   (semseg/dino_segmentor.py, backbone/dinov2.py, __init__.py's)
    core/    (inference.py, __init__.py)
    examples/ (a few demo images, prefixed pascal_/cityscapes_/ade20k_)

Weights are pulled per dataset from the matching HF model repo (see DATASETS). Each released
checkpoint is the EMA teacher of the rule CW-BASS v2's reliability gate SELECTS on that dataset:
strict filtering on the saturated Pascal/Cityscapes teachers, the adaptive floor on the
confidently-unreliable ADE20K teacher. Runs on CPU (free tier); uses CUDA if present. Set
CWBASS_LOCAL_DIR to load weights from a local folder (filenames must match DATASETS[*]['file'])
and CWBASS_NO_WARM=1 to skip background warming — both used for offline testing.
"""
...
DATASETS = {
    "Pascal VOC (21 classes)": dict(
        repo="psychofict/cwbass-v2-pascal", file="cwbassv2_pascal_dinov2b_1over8.pth",
        nclass=21, classes=VOC_CLASSES, palette=VOC_PALETTE, bg=0),
    "Cityscapes (19 classes)": dict(
        repo="psychofict/cwbass-v2-cityscapes", file="cwbassv2_cityscapes_dinov2b_1over8.pth",
        nclass=19, classes=CS_CLASSES, palette=CS_PALETTE, bg=None),
    "ADE20K (150 classes)": dict(
        repo="psychofict/cwbass-v2-ade20k", file="cwbassv2_ade20k_dinov2b_1over8.pth",
...
def _weights_path(cfg):
    local_dir = os.environ.get("CWBASS_LOCAL_DIR")
    if local_dir:
...
def get_model(dataset):
    ...
    with _LOCK:
        if dataset not in _MODELS:
            ...
            model = DINOv2Segmentor(backbone="dinov2_vitb14", nclass=cfg["nclass"],
                                    pretrained=False).eval()
            # strict=False: the training-only contrastive proj_head is absent from the released
            # weights; the backbone / DPT-lite decoder / segmentation head all load.
...
#hero h1 {font-size: 2rem; font-weight: 800; margin: 0 0 2px;
  background: linear-gradient(90deg,#0ea5e9,#6366f1); -webkit-background-clip:text;
...
<div id="hero">
  <h1>CW-BASS v2</h1>
  <div class="sub">Saturation-Aware Pseudo-Label Selection for Semi-Supervised Segmentation under Foundation-Model Teachers</div>
  <div class="links">
    <a href="https://psychofict.github.io/CW-BASS-v2/" target="_blank">🌐 Project page</a>
    <a href="https://github.com/psychofict/CW-BASS-v2" target="_blank">💻 Code</a>
    <a href="https://huggingface.co/psychofict/cwbass-v2-pascal" target="_blank">Pascal VOC</a>
    <a href="https://huggingface.co/psychofict/cwbass-v2-cityscapes" target="_blank">Cityscapes</a>
    <a href="https://huggingface.co/psychofict/cwbass-v2-ade20k" target="_blank">ADE20K</a>
  </div>
  <div class="hint">DINOv2-Base encoder. Each label set loads the checkpoint of the rule the reliability
  gate selects — strict on Pascal/Cityscapes, adaptive floor on ADE20K. Upload an image and pick a
  label set, or click an example. The first run per model warms up (~1 min on CPU); after that it is fast.</div>
</div>
"""

FOOT = "<div id='foot'>CW-BASS v2 · single DINOv2-Base backbone · reliability-gated pseudo-label selection · no inference-time parameters</div>"

with gr.Blocks(theme=gr.themes.Soft(primary_hue="sky", secondary_hue="indigo"),
               css=CSS, title="CW-BASS v2 — Semi-Supervised Segmentation Demo") as demo:
    gr.HTML(HERO)
    with gr.Row():
...
if os.environ.get("CWBASS_NO_WARM") != "1":
